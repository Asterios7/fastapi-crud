...
def vote(vote: schemas.Vote, 
    ...
    # Cannot Vote on a post that doesn't exist
    ...
    vote_query = db.query(models.Vote).filter(models.Vote.post_id == vote.post_id,
                                     models.Vote.user_id == current_user.id) # Check if there is a vote for this post id, and 2nd check also if its done by the same person
